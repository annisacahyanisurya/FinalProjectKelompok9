...
BEGIN
    ...
    IF @street_address IS NULL 
		...
		WHERE id = @id
	ELSE IF (SELECT [dbo].func_street_address_length(@street_address)) = 1
	...
	ELSE IF @city IS NULL 
		...
		SET street_address = @street_address, postal_code = @postalcode, city = city, state_province = @state_province, country = @country
		WHERE id = @id
	ELSE IF (SELECT [dbo].func_city_format(@city)) = 1
	...
	ELSE IF @state_province IS NULL 
		...
		SET street_address = @street_address, postal_code = @postalcode, city = @city, state_province = state_province, country = @country
		WHERE id = @id
	ELSE IF (SELECT [dbo].func_state_province_format(@state_province)) = 1
...
EXEC usp_edit_location 'Jl.Melati No.14', '15336', 'Tangerang Selatan', 'Banten', 'JP', 3300
